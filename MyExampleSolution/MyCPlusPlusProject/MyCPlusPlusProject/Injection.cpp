...
#if defined(_WIN64)
#define DBG_HELP_DLL _T("dbg64.dll")
#else
#define DBG_HELP_DLL _T("dbg32.dll")
#endif

#if defined(_WIN64)
#define SYMCHK_EXE _T("symchk.exe")
#else
#define SYMCHK_EXE _T("symchk.exe")
#endif
...
DWORD __stdcall CInjection::Initialize(LPVOID lpParameter)
{
	...
	if (!CSymbolAddressCache::TryCache())
	{
		...
		// strCurrentDir actually points back to the test project library release folder (C:\Users\tbowen\Desktop\MyExampleSolution\MyExampleSolution\MyClassLibrary.Tests\bin\x64\Release).
	...
	if (p_getJit)
	{
		...
		{
			//			ICorJitCompiler* yy= new ICorJitCompiler();
			DWORD OldProtect;
			VirtualProtect(ICorJitCompiler::Instance, sizeof(ULONG_PTR), PAGE_READWRITE, &OldProtect);
			compileMethodcache = ICorJitCompiler::Instance->ñompileMethodintercept;
			ICorJitCompiler::Instance->ñompileMethodintercept = &ICorJitCompiler::compileMethod;
